...
export const getSubscriberGroupByCode = async (code: string): Promise<Result<SubscriberGroupModel>> => {
  try {
    const result = await SubscriberGroup.findOne({ where: { code } })

    if (!result) {
      throw new Error('Subscriber group not found')
    }

    console.log('Subscriber group found successfully:', result);
    return {
      successful: true,
      data: result,
      error: null
